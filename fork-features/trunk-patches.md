...
| `shared/src/apiTypes.ts`、`hub/src/web/routes/machines.ts`（+test）、`web/src/api/client.ts`、`web/src/hooks/mutations/useSpawnSession.ts` | spawn 请求无扩展字段注册点 | `SpawnSessionRequestSchema.ccSwitchProviderId` 一路透传到 `engine.spawnSession` 已有的第 14 个参数（runner 侧 `getCcSwitchProviderLaunchEnv` 早已支持）；仅 Claude 生效 | New Session 选供应商 → spawn body → hub → runner 给该子进程注入供应商 env | hub 路由单测；真机：吹雪3080（当前 GLM）选 cx2cc 供应商 + astra 建会话，回复 model=gpt-6-astra |
| `cli/src/runner/run.ts`、`cli/src/modules/common/hooks/generateHookSettings.ts`（+test） | wrapper 的钩子设置文件是 `~/.claude/settings.json` 的整份副本（含 cc-switch 写入的当前供应商 `env`），`--settings` 是命令行级设置，其 env 盖过进程 env——runner 只注入进程 env 的既有做法在 cc-switch 托管机器上必然失效（生产实测：选 cx2cc 供应商仍跑 glm-5.3） | runner 额外交 `HAPI_CLAUDE_SETTINGS_ENV_OVERRIDE`（JSON），`buildHookSettings` 用它覆盖 `settings.env`；**要机群 CLI 换芯才生效**，随下一次 CLI 换芯发布 | spawn(ccSwitchProviderId) → runner env → wrapper 钩子设置 env → Claude Code 打到所选供应商 | 换芯后：吹雪3080（当前 GLM）选 cx2cc 供应商 + astra 建会话，回复 model=gpt-6-astra；单测 4 例 |
| `web/src/components/NewSession/index.tsx`（供应商选择器） | 同上 | `useCcSwitchProviders(machineId)` + fork 组件 `CcSwitchProviderSelector`，按机器记忆（`hapi:newSession:ccSwitchProvider:v1:<machineId>`）；只在选择≠当前供应商时才发 id | 同上 | 单测 4 例（隐藏/默认不发/发非当前/记忆==当前不发） |
...
> 修它的动因：目录限定上线后这个旧洞的危害升级——被抢到的是 **owner 档位，直接
> 越过 path_prefix**，一条未绑定会话就能把限定架空。审计时全量联查（2026-08-11
> 深夜）：当前 0 条未绑定会话（admin 客户端拉表频繁，稳态下都被及时绑掉），暴露
> 只存在于「CLI 建会话 → 下一次整表拉取」的竞态窗口，8-10 那 15 条就是竞态输掉的
> 结果。5 个用例钉住（不抢/主人认领/admin 列表绑给机器主人/未注册机器保持原行为/
> 限定外不落绑定）。以下为原始记录。
