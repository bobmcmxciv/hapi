...
const AGENT_SESSION_ID_FIELD_BY_FLAVOR = {
    ...
    kimi: 'kimiSessionId',
    pi: 'piSessionId',
    // OMP 的原生 id 不在扁平字段上，而在 ompSession 快照里；
    // getSummaryAgentSessionId 对 omp 单独取 ompSession.id。
    omp: 'ompSession'
...
function getSummaryAgentSessionId(metadata: Metadata): string | undefined {
    ...
    if (flavor === 'omp') {
        const nativeId = metadata.ompSession?.id
        return typeof nativeId === 'string' && nativeId.trim() ? nativeId.trim() : undefined
    }
